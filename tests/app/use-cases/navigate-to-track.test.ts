...
const makeRow = () => ({}) as HTMLTableRowElement;
...
const makeBcPlayer = (
  ...
  prevBtn = makeBtn(),
  nextBtn = makeBtn(),
  playabilityMap?: boolean[]
): BcPlayerPort =>
  ({
    getTrackTitle: (_pageType: string) => currentTitle,
    getTrackPlayabilityMap: () => playabilityMap ?? rows.map(() => true),
    getTrackRows: () => rows,
...
describe("navigateToTrack", () => {
  ...
  it("does not navigate when the current track is unknown", () => {
    const prev = makeBtn();
    const next = makeBtn();
    const rows = [makeRow(), makeRow(), makeRow()];
  ...
  it("does not navigate when the index is negative", () => {
    const prev = makeBtn();
    const next = makeBtn();
    navigateToTrack(-1, makeBcPlayer([makeRow()], ["A"], "A", prev, next));
    expect(prev.click).not.toHaveBeenCalled();
    expect(next.click).not.toHaveBeenCalled();
  });
  ...
  it("does not navigate when the track is unplayable", () => {
    const prev = makeBtn();
    const next = makeBtn();
    navigateToTrack(0, makeBcPlayer([makeRow()], ["A"], null, prev, next, [false]));
    expect(prev.click).not.toHaveBeenCalled();
    expect(next.click).not.toHaveBeenCalled();
  });
