 ...
 *   Pass 2 — All other scripts, checked in priority order, first match wins.
 *
...
const UNDETERMINED = ""; // used for Latin chars and unrecognised code points
const SCRIPT_RANGES: ReadonlyArray<readonly [number, number, string]> = [
  [0x0041, 0x005a, UNDETERMINED], // Latin uppercase     → undetermined
  [0x0061, 0x007a, UNDETERMINED], // Latin lowercase     → undetermined
  [0x0080, 0x00ff, UNDETERMINED], // Latin-1 Supplement  → undetermined
  [0x0100, 0x017f, UNDETERMINED], // Latin Extended-A    → undetermined
  [0x0180, 0x024f, UNDETERMINED], // Latin Extended-B    → undetermined
...
export const detectScriptLang = (text: string): string => {
  ...
  const scriptsByMostPresent = Object.entries(scriptsFound).reduce((a, b) => (a[1] > b[1] ? a : b), ["", 0]);
  return scriptsByMostPresent[0] || UNDETERMINED;
};
