...
enum BC_ELEM_IDENTIFIERS {
  ...
  onTrackCurrentArtist = "div.name-section",
  audioPlayer = "audio",
...
(() => {
  ...
  const cielchToRGB = ([L, C, H]: [number, number, number]): string => {
    const oklchToRGB = ([L, C, H]: [number, number, number]): string => {
    ...
    const cielchToOklch = ([L, C, H]: [number, number, number]): [number, number, number] => {
      // --- CIELCH → CIELAB ---
      const hRad = (H / 180) * Math.PI;
      const a = C * Math.cos(hRad);
      const b = C * Math.sin(hRad);
    ...
    const asOKLCH = cielchToOklch([L, C, H]);
    return oklchToRGB(asOKLCH);
  };
  ...
  const getArtistNameElement = (): HTMLSpanElement => {
    const nameSection = document.querySelector("div#name-section") as HTMLElement;
    const nameSectionLinks = nameSection.querySelectorAll("span");
    const artistNameIndex = nameSectionLinks.length - 1; // 0 if album page, 1 if track page
    return nameSectionLinks[artistNameIndex].querySelector("a")! as HTMLSpanElement;
  };
  ...
  const getAppropriatePretextColor = (): string => {
    ...
    return cielchToRGB([CIELCH_MAGIC_LIGHTNESS, CIELCH_MAGIC_CHROMA, preferredColorOKLCH[2]]);
  };
